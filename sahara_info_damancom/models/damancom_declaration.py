...
class DamancomDeclaration(models.Model):
    ...
    def action_load_payslips(self):
        ...
        for payslip in payslips:
            ...
            # ALLOC_FAM a result = -(SBC * 0.0640) donc total est négatif → abs()
            af_a_payer = abs(af_line[:1].total) if af_line else 0.0
            af_a_deduire = 0.0
            ...
            if situation in ('CS', 'MS'):
                ...
                af_a_reverser = 0.0
            nb_enfants = 0
            for fname in ('l10n_ma_personnes_charge', 'children'):
                if fname in employee._fields:
                    nb_enfants = employee[fname] or 0
                    break
    ...
    def action_validate(self):
        ...
        for line in self.line_ids:
            emp_name = line.employee_id.name or line.nom_prenom
            if not line.is_entrant:
                if line.num_immatriculation in ('000000000', ''):
                    errors.append(_("Employe %s: N d'immatriculation CNSS manquant.") % emp_name)
                else:
                    valid, msg = validate_num_immatriculation(line.num_immatriculation)
                    if not valid:
                        errors.append(_("Employe %s: N immatriculation invalide (%s) - %s") % (emp_name, line.num_immatriculation, msg))
            if line.jours_declares > 26:
                errors.append(_("Employe %s: jours declares (%d) > 26") % (emp_name, line.jours_declares))
            if line.situation in ('CS', 'MS') and (line.jours_declares > 0 or line.salaire_reel > 0):
                errors.append(_("Employe %s: situation %s exige jours=0 et salaire=0") % (emp_name, line.situation))
            if line.num_immatriculation in ('000000000', '') and not line.num_cin:
                errors.append(_("Employe %s: N CIN obligatoire pour les entrants sans N d'immatriculation.") % emp_name)
            if line.salaire_plafonne > line.salaire_reel:
                errors.append(_("Employe %s: salaire plafonne (%s) > salaire reel (%s)") % (emp_name, line.salaire_plafonne, line.salaire_reel))
        if errors:
...
class DamancomDeclarationLine(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employe')
