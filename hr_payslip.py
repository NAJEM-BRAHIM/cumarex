...
class DamancomDeclaration(models.Model):
    ...
    # Préétabli (importé depuis CNSS)
    identif_transfert = fields.Char(
        string="Identifiant Transfert (CNSS)",
        size=14,
        help="Identifiant fourni par la CNSS dans le fichier préétabli. "
             "Doit être recopié à l'identique dans le fichier BDS généré.",
    )
    preetabli_imported = fields.Boolean(
        string="Préétabli importé",
        default=False,
        readonly=True,
        help="Indique qu'un fichier préétabli AFFEBDS a été importé pour "
             "cette déclaration.",
    )
    preetabli_file = fields.Binary(
        string="Fichier Préétabli AFFEBDS",
        attachment=True,
        help="Copie du fichier préétabli importé (pour traçabilité).",
    )
    preetabli_filename = fields.Char(string="Nom Fichier Préétabli")
    ...
    def action_load_payslips(self):
        ...
        for payslip in payslips:
            ...
            # Allocations Familiales depuis la fiche de paie
            ...
            lines_data.append((0, 0, {
                ...
                'af_net_a_payer': af_net_a_payer,
                'af_a_reverser': af_a_reverser,
    ...
    def action_validate(self):
        ...
        for line in self.line_ids:
            ...
            if line.num_immatriculation in ('000000000', '') and not line.num_cin:
                errors.append(_(
                    "Employé %s: N° CIN obligatoire pour les entrants sans N° d'immatriculation."
                ) % line.employee_id.name)
    ...
    def _build_record_b00(self):
        ...
        # Identif_Transfert : DOIT être identique à celui du préétabli reçu de CNSS
        if self.identif_transfert and self.identif_transfert.strip():
            # Utiliser l'ID importé du préétabli
            identif_transfert = self.identif_transfert.strip().zfill(14)[:14]
        else:
            # Pas de préétabli importé : 0 (sera signalé par le portail)
    ...
    def _build_record_b04_empty(self):
        ...
        content = (
            ...
            format_centimes(0, 13) +   # sal_reel = 0
            format_centimes(0, 9) +    # sal_plaf = 0
